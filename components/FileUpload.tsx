...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelect, onTextSubmit, onLoadSample }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {[
            ...
            { label: '20', val: 20 },
            { label: '30', val: 30 },
            { label: '50 preg. ⚡', val: 50 },
            { label: '100 preg. 🔥', val: 100 },
          ].map((item, i) => (
            <button
              key={i}
              type="button"
      ...
      {/* Helper text if 50 or 100 selected */}
      {(targetCount === 50 || targetCount === 100) && (
        <div className="text-xs text-blue-800 bg-blue-50 border border-blue-200/80 rounded-xl px-4 py-2.5 flex items-center justify-between animate-fadeIn">
          <span>
            <strong>Modo documento extenso ({targetCount} preguntas):</strong> La IA profundizará en todos los epígrafes, artículos y detalles de tu documento.
          </span>
          <span className="text-[11px] font-semibold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-md">
            Extenso
          </span>
        </div>
      )}
